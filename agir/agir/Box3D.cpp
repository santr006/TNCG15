...
Intersection* Box3D::testRayIntersection(Ray &r, float step, glm::vec3 &intersectionPoint)
{
	...
	Intersection* closest = front.testRayIntersection(r, step, intersectionPoint);

	Intersection* current = back.testRayIntersection(r, step, intersectionPoint);
	if (current != nullptr)
		closest = current;

	current = right.testRayIntersection(r, step, intersectionPoint);
	if (current != nullptr)
		closest = current;

	current = left.testRayIntersection(r, step, intersectionPoint);
	if (current != nullptr)
		closest = current;

	current = top.testRayIntersection(r, step, intersectionPoint);
	if (current != nullptr)
		closest = current;

	current = bottom.testRayIntersection(r, step, intersectionPoint);
	if (current != nullptr)
		closest = current; 
